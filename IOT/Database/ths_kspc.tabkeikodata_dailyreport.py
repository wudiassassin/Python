"""
表名:每日采集器情报
功能:从ths_proces.tabkeikodata_everytime获取8月份每天的第一条数据
...
数据示例：
2022-08-01 15:48:26	EM-XL-001	EM-XL-001	V	016	374	1	373.0	187.5	375.0	14	1	12	30.0	40.0	50.0	60.0	70.0	100.0
"""
...
while True:
    ...
    if r != "exit":
        ...
        for i in range(1, 32):
            for j in ['1919', '1920', '017', '016']:
                sql = "SELECT " + \
                      ...
                      "FROM tabkeikoalert " + \
                      "WHERE sensorid = '" + str(r) + "' " + \
                      "AND to_char(regstdatetime,'yyyy-MM-dd HH24:MI:ss') LIKE '%2022-08-" + '%02d' % i + "%' " + \
                      "AND alertstatus = '02'" + \
                      "AND dispstatus = '" + str(j) + "'" + \
                      "UNION ALL " + \
                      "SELECT " + \
        ...
        cur.copy_from(f, 'tabkeikodata_dailyreport', null='', columns=('targetdate', 'eqpid', 'sensorid', 'unit',
